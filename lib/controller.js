...
module.exports.client = function() {
  var $, THREE, animate, aspect, camera, cameraRevolveAngle, cameraRevolveAxis, cameraRevolveMatrix, container, dom, element, far, fov, height, history, near, pointLight, pointerlockchange, pointerlockerror, radius1, radius2, renderer, rings, scene, segments, sphere1Geometry, sphere2Geometry, sphereMaterial, spheres, t, width;
  THREE = require('three');
  $ = require('jquery');
  console.log($);
  return;
  ...
  animate = function() {
    var G, distanceScalar, forceScalar, forceVector, historyGeometry, historyMaterial, i, j, massSum, position1, position2, vertices, _i, _j, _ref;
    try {
      ...
      for (i = _i = 0; _i <= 1; i = ++_i) {
        ...
        while (vertices.length > history) {
          vertices.shift();
        }
        scene.remove(spheres[i].historyLine);
        historyMaterial = new THREE.LineBasicMaterial({
          color: 0x888888
        });
        historyGeometry = new THREE.Geometry;
        try {
          for (j = _j = 0, _ref = vertices.length - 1; 0 <= _ref ? _j <= _ref : _j >= _ref; j = 0 <= _ref ? ++_j : --_j) {
